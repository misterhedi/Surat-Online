...
export const DocumentPreview: React.FC<DocumentPreviewProps> = ({ submission, template, previewHtml }) => {
  ...
  const renderLocalHtml = () => {
    ...
    template.fields_config.forEach(field => {
      ...
      html = html.replace(regex, `<span class="border-b border-[#E5E1DA] px-1 font-semibold text-[#1C1A17]">${val}</span>`);
    });
    ...
    <div className="relative bg-white shadow-inner rounded-md border border-[#E5E1DA] overflow-hidden">
      ...
      <div className="absolute top-4 right-4 z-10 no-print flex items-center gap-2">
        <button
          ...
          className="bg-white hover:bg-[#FAF9F5] text-[#1C1A17] border border-[#E5E1DA] rounded px-3 py-1.5 text-[10px] font-mono font-bold uppercase tracking-wider shadow-sm flex items-center gap-1.5 transition cursor-pointer hover:border-[#1C1A17]"
          title="Cetak Surat Resmi (A4)"
        >
